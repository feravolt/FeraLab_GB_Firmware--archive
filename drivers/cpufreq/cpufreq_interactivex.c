...
//FIX: now going to max policy value //#define FREQ_THRESHOLD 998400;
...
{
	unsigned int cpu;
//FIX: now going to max policy value //	unsigned int newtarget;
	cpumask_t tmp_mask = work_cpumask;
//FIX: now going to max policy value //	newtarget = FREQ_THRESHOLD;
...
//FIX: now going to max policy value //			__cpufreq_driver_target(policy, newtarget, CPUFREQ_RELATION_H);
		} else {
